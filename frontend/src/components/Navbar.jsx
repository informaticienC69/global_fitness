  ...
  return (
    <header
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${
        scrolled
          ? 'glass py-3 shadow-[0_4px_30px_rgba(0,0,0,0.6)]'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="container mx-auto px-6 md:px-10 flex justify-between items-center">
        ...
        <a href="#" className="flex items-center gap-2.5 group select-none flex-shrink-0">
          <div className="relative w-8 h-8 rounded-md bg-brand-gradient flex items-center justify-center shadow-[0_0_12px_rgba(230,57,70,0.5)] flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
          ...
          <div className="font-display leading-none">
            ...
            <span className="text-lg font-light text-white/60 tracking-[0.06em] ml-1">FITNESS</span>
          </div>
        </a>
        ...
        <nav className="hidden md:flex gap-5 items-center">
          {links.map(link => (
            <a
              key={link.href}
              href={link.href}
              className="text-gray-300 hover:text-white font-body text-xs font-600 uppercase tracking-[0.15em] transition-colors relative group"
            >
              {link.label}
          ...
          <div className="flex items-center gap-4 ml-4">
            <button onClick={toggleDrawer} className="relative text-gray-300 hover:text-white transition-colors">
              ...
              {totalItems > 0 && (
                <span className="absolute -top-1.5 -right-2 bg-brand text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                  {totalItems}
                </span>
              )}
            </button>
            <a href="#contact" className="btn-primary py-2.5 px-6 text-[11px]">
              Essai Gratuit
            </a>
          </div>
        </nav>
